...
semantic_plot <- function(file, title = "My Semantic Plot", subtitle, disp = 0.15) {
  ...
  revigo_tib %>% 
    ...
    geom_point(aes(plot_x, plot_y, size = log_size), shape = 21, fill = "transparent", color = I(alpha("black", 0.6))) + 
    scale_size(range=c(5, 15)) + 
    geom_text_repel(data = ex, aes(plot_x, plot_y, label = name), color = I(alpha("black", 0.85)), size = 6) +
